...
function modifyRelation() {
    ...
    if (dom.attr("relation") === 'false') {
        //添加关注
        $.post("/home/modifyRelation", {uid: uid, rid: rid, status: 1}, function (data) {
            if (data.status === 2007) {
        ...
        //取消关注
        $.post("/home/modifyRelation", {uid: uid, rid: rid, status: 0}, function (data) {
            if (data.status === 2008) {
...
function chat() {
    ...
    var indexPrompt = layer.prompt({
        ...
        $.post("/message/send", {uid: uid, rid: rid, content: con }, function (data) {
            if (data.status === 2005) {
                //关闭发送框
