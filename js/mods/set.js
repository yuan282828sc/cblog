                    ...
                    if (data.status === 1012) {
                        ...
                        Countdown = setInterval(function () {
                            ...
                            if (current_time < 1) {
                                ...
                                // window.location.href = "/login"; //#设定跳转的链接地址
                                window.location.href = "/logout"; //#设定跳转的链接地址
                            }
                        }, 1000);
                    } else {
...
//普通图片上传
    var uploadInst = upload.render({
        elem: '#uploadAvatar'
        , url: '/upload/avatar/'+uid //改成您自己的上传接口
...
function sendMessage() {
    ...
    $.get("/reg/sendEmail", {"email": email, "emailStatus": '1001'}, function (data) {
        if (data.status === 1005) {
            //发送成功
            layer.msg(data.message, {time: 2000, icon: 6});
        } else {
