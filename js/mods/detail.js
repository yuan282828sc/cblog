...
function likeMark(index) {
    ...
    if (dom.attr("data-like") === 'false') {
        //点赞
        $.post("http://cblog.com/article/comment/likeMark", {"mark": 1, "id": commentId, "uid": commentatorId}, function (data) {
            if (data.status == 3005) {
        ...
        //取消点赞
        $.post("http://cblog.com/article/comment/likeMark", {"mark": 0, "id": commentId, "uid": commentatorId}, function (data) {
            if (data.status == 3006) {
                layer.msg(data.message);
...
function delBlog(aid){
    ...
    layer.confirm('您确定要永久删除帖子？', {
        ...
        //确定
        $.post("/atricle/del", {"aid": aid}, function (data) {
            if (data.status === 3010) {
...
function collect() {
    ...
    if (dom.attr("data-collect") === 'false') {
        //收藏
        $.post("/article/collect", {"mark": 1, "aid": aid, "uid": uid}, function (data) {
            if (data.status === 3012) {
                layer.msg(data.message);
        ...
        //取消收藏
        $.post("/article/collect", {"mark": 0, "aid": aid, "uid": uid}, function (data) {
            if (data.status === 3013) {
                layer.msg(data.message);
...
function topBlog() {
    ...
    if (dom.attr("data-top") === 'false') {
        //置顶
        $.post("http://user.cblog.com/admin/setTop", {"top": 1, "aid": aid}, function (data) {
            if (data.status === 6001) {
                layer.msg(data.message);
        ...
        //取消置顶
        $.post("http://user.cblog.com/admin/setTop", {"top": 0, "aid": aid}, function (data) {
            if (data.status === 6001) {
                layer.msg(data.message);
...
//加精帖子
        $.post("http://user.cblog.com/admin/setBoutique", {"boutique": 1, "aid": aid}, function (data) {
            if (data.status === 6002) {
                layer.msg(data.message);
        ...
        //取消加精帖子
        $.post("http://user.cblog.com/admin/setBoutique", {"boutique": 0, "aid": aid}, function (data) {
            if (data.status === 6002) {
                layer.msg(data.message);
